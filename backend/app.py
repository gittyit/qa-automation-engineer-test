...
async def result():
    ...
    try:
        ...
        msg = add_msg("Can't retrieve data from DB tables", Ex)
        return Render("error.html", error_msg = msg, ex_msg = Ex)
    return jsonify(d)
...
async def run():
    ...
    try:
        ...
        msg = add_msg("Can't delete DB tables", Ex)
        return Render("error.html", error_msg = msg, ex_msg = Ex)
    ...
    try:
        ...
        msg = add_msg("Can't create DB tables", Ex)
        return Render("error.html", error_msg = msg, ex_msg = Ex)
...
async def populate_table(connection, tablename, sequence1: None, sequence2: None):
    try:
        ...
        msg = add_msg(f"Can't insert data into table {tablename}", Ex)
        return Render("error.html", error_msg = msg, ex_msg = Ex)